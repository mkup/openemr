...
include_once("$srcdir/gen_accident_form.inc.php");
...
if (isset($_POST['bn_process_hcfa']) OR
        isset($_POST['bn_wc_followup']) OR
        isset($_POST['bn_therapy_notes']))
    $bat_filename .= 'pdf';
else
if (isset($_POST['bn_hcfa_txt_file']))
    $bat_filename .= 'txt';
else
if (isset($_POST['bn_nf_initial']) OR
        isset($_POST['bn_nf_followup']))
    $bat_filename .= 'rtf';
    ...
    foreach ($ar['claims'] as $claimid => $claim_array) {
        ...
        if (isset($claim_array['bill'])) {
            ...
            if (!$tmp) {
                ...
                } else if (isset($ar['bn_nf_initial']) OR
                        isset($ar['bn_nf_followup']) OR
                        isset($ar['bn_wc_followup']) OR
                        isset($ar['bn_therapy_notes'])) {
//  
                    if (isset($ar['bn_nf_initial'])) $form = 'NF3';
                    else if(isset($ar['bn_nf_followup'])) $form = 'NF3FU';
                         else if (isset($ar['bn_wc_followup'])) $form = 'C42';
                              else if (isset($ar['bn_therapy_notes'])) $form = 'PTN';
                    $log = '';
                    $lines = gen_accident_form($patient_id, $encounter, $log, $form);
                    fwrite($hlog, $log);
                    $bat_content .= $lines;
                    if (!updateClaim(false, $patient_id, $encounter, -1, -1, 2, 2, $bat_filename)) {
                        $bill_info[] = xl("Internal error: claim ") . $claimid . xl(" not found!") . "\n";
    ...
    if (isset($ar['bn_hcfa_txt_file']) OR
            isset($ar['bn_nf_initial']) OR
            isset($ar['bn_nf_followup']) OR
            isset($ar['bn_wc_followup']) OR
            isset($ar['bn_therapy_notes'])) {
        fclose($hlog);
